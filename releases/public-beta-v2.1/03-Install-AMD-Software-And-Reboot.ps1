...
function Get-ConflictingNative2099MsiRecords {
    return @(
        Get-AmdSettingsRecords |
            Where-Object {
                $_.DisplayVersion -match '\.2099$' -and
                -not (
                    $_.KeyName -ieq $ExpectedNativeMsiProductCode -and
                    $_.DisplayVersion -eq $ExpectedNativeMsiDisplayVersion
                )
...
function Install-Or-RepairNativeMsi {
    ...
    $ConflictingNativeRecords = @(Get-ConflictingNative2099MsiRecords)
    foreach ($ConflictingRecord in $ConflictingNativeRecords) {
        $ConflictingProductCode = [string]$ConflictingRecord.KeyName
        $ConflictingVersion = [string]$ConflictingRecord.DisplayVersion
        $ConflictStamp = ($ConflictingProductCode -replace '[^A-Fa-f0-9]', '')
        $ConflictLog = Join-Path `
            $LogRoot `
            "05-Conflicting-2099-Uninstall-$ConflictStamp-$Timestamp.log"

        Write-Host (
            'Removing conflicting AMD Settings .2099 before installing the ' +
            "exact bundled build: $ConflictingVersion / $ConflictingProductCode"
        )
        ...
        if ([int]$ConflictProcess.ExitCode -notin @(0, 1605, 3010)) {
            throw (
                'Conflicting AMD Settings .2099 uninstall failed with exit ' +
                "code $($ConflictProcess.ExitCode). Log: $ConflictLog"
            )
        }
    }

    if (@(Get-ConflictingNative2099MsiRecords).Count -gt 0) {
        throw 'A conflicting AMD Settings .2099 MSI remains registered.'
    }
