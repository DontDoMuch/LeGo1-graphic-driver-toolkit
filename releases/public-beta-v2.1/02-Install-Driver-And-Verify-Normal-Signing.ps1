...
<#
.SYNOPSIS
    Script 2 of 4 for the Legion Go AMD 26.6.4 Toolkit.

.DESCRIPTION
    Installs the corrected AMD 26.6.4 display driver, disables Test Signing
    for the next Windows boot, and verifies the driver after restart under
    normal Windows signing.

    First run:
      - verifies Script 1 state and the compatible Legion Go starting state;
...
.NOTES
    Public Beta v2.0. Accepts a healthy recorded prior AMD display state and a dynamically recorded AMD-signed CCC2 extraction wrapper.
...
function Request-WindowsRestart {
    ...
    & shutdown.exe `
        ...
        /t 10 `
        /c (
            'Legion Go AMD 26.6.4 Toolkit: restarting to verify the ' +
...
$EmbeddedPayload = [ordered]@{
    ...
    'Phase-01-Install-Corrected-Driver.ps1' = [ordered]@{
        SHA256 = 'DA5C6150ACC04BA01A41CA81AD226787E9D8026E52BC2ACA602836F930F9562A'
        Utf8Bom = $true
        LineEnding = 'CRLF'
...
<#
Phase 1 of Script 2 in the Legion Go AMD 26.6.4 Toolkit.
...
Preconditions:
  ...
  - A healthy supported Legion Go AMD display stack is active and matches
    the starting state recorded by Script 1, unless the exact target driver
    is already active from a partial rerun.
...
Actions:
  ...
  - Verify the recorded compatible starting state and required extension.
  - Import the per-user public certificate into LocalMachine Root and
    TrustedPublisher.
  - Stage and force-bind the corrected AMD 26.6.4 display package.
...
param(
    ...
    [string]$BootPreparationResultPath =
        'C:\ProgramData\LegionGo-AMD-26.6.4\boot-preparation-result.json',
...
$ExpectedTargetVersion = '32.0.31021.5001'
...
function Resolve-DriverPackageRoot {
    ...
    foreach ($Candidate in $Candidates) {
        ...
        $DirectInf = Join-Path $CandidatePath 'u0202082.inf'
        $LegacyInf = Join-Path $CandidatePath 'Driver\u0202082.inf'
...
function Get-GpuDriver {
    ...
    # Resolve the device instance through Win32_PnPEntity, read the active INF
    # and driver version from the live Enum/Class binding, and project signing
    # metadata only from a signed-driver record that exactly matches that live
    # DeviceID, INF, and version.
    $Devices = @(
        Get-CimInstance Win32_PnPEntity |
            Where-Object DeviceID -Like $GpuPattern
    )
    ...
    $SignedDriver = @(
        Get-CimInstance Win32_PnPSignedDriver |
            Where-Object {
                [string]$_.DeviceID -ieq [string]$Device.DeviceID -and
                [string]$_.InfName -ieq $InfName -and
                [string]$_.DriverVersion -eq $DriverVersion
            }
    ) | Select-Object -First 1

    $DriverProviderName = ''
    $DriverSigner = ''
    $DriverIsSigned = $false

    if ($null -ne $SignedDriver) {
        $ProviderProperty =
            $SignedDriver.PSObject.Properties['DriverProviderName']
        $SignerProperty =
            $SignedDriver.PSObject.Properties['Signer']
        $IsSignedProperty =
            $SignedDriver.PSObject.Properties['IsSigned']

        if ($null -ne $ProviderProperty) {
            $DriverProviderName = [string]$ProviderProperty.Value
        }

        if ($null -ne $SignerProperty) {
            $DriverSigner = [string]$SignerProperty.Value
        }

        if ($null -ne $IsSignedProperty) {
            $DriverIsSigned = [bool]$IsSignedProperty.Value
        }
        elseif (-not [string]::IsNullOrWhiteSpace($DriverSigner)) {
            # Some CIM projections omit IsSigned while still exposing the
            # Windows-recorded signer. Preserve the signed state only when the
            # exact live DeviceID/INF/version record supplied that signer.
            $DriverIsSigned = $true
        }
    }
...
try {
    Write-Host 'Legion Go AMD 26.6.4 Toolkit'
    ...
    $DriverInf = Join-Path $DriverPackageRoot 'u0202082.inf'
    $LocalCatalog = Join-Path $DriverPackageRoot 'u0202082.cat'
    $KernelSource = Join-Path $DriverPackageRoot 'B026218\amdkmdag.sys'
    $AmgcfPath = Join-Path $DriverPackageRoot 'B026218\amdgcf.dat'
    $AtiicdxxPath = Join-Path $DriverPackageRoot 'B026218\atiicdxx.dat'
    $UnexpectedPackageCcc2Path =
        Join-Path $DriverPackageRoot 'B026218\ccc2_install.exe'
    ...
    if (
        ...
        throw (
            ...
            'B026218\ccc2_install.exe. The validated package boundary is ' +
    ...
    if (@($OfficialCcc2Candidates).Count -eq 0) {
        throw (
            ...
            'separate official AMD 26.6.4 ccc2_install.exe.'
        )
    }
    ...
    $RecordedOfficialCcc2Hash =
        [string](Get-FirstPropertyValue `
            -Object $SigningInput `
            -Names @('OfficialCcc2SHA256'))

    if ([string]::IsNullOrWhiteSpace($RecordedOfficialCcc2Hash)) {
        $RecordedOfficialCcc2Hash =
            [string](Get-FirstPropertyValue `
                -Object $Verification `
                -Names @('OfficialCcc2SHA256'))
    ...
    $RecordedOfficialCcc2Length =
        Get-FirstPropertyValue `
            -Object $SigningInput `
            -Names @('OfficialCcc2Length')

    if ($null -eq $RecordedOfficialCcc2Length) {
        $RecordedOfficialCcc2Length =
            Get-FirstPropertyValue `
                -Object $Verification `
                -Names @('OfficialCcc2Length')
    ...
    if (
        ...
        $null -eq $RecordedOfficialCcc2Length -or
        [int64]$RecordedOfficialCcc2Length -le 0
    ) {
        throw (
            'Script 1 state did not record the accepted CCC2 wrapper ' +
            'hash and length.'
        )
    }
    ...
    if ([string]::IsNullOrWhiteSpace($ResolvedOfficialCcc2)) {
        throw (
            'Unable to resolve the AMD-signed ccc2 extraction wrapper ' +
            'recorded by Script 1.'
        )
    }
    ...
    $OfficialCcc2Hash =
        Get-SHA256 -LiteralPath $ResolvedOfficialCcc2
    $OfficialCcc2Length = [int64]$OfficialCcc2Item.Length

    if ($OfficialCcc2Length -ne [int64]$RecordedOfficialCcc2Length) {
        throw (
            'The recorded ccc2_install.exe length changed. Recorded ' +
            [string]$RecordedOfficialCcc2Length + '; actual ' +
            [string]$OfficialCcc2Length + '.'
        )
    }
    ...
    Write-Host "       SHA256: $OfficialCcc2Hash"
    ...
    $OfficialCatalogHash =
        Assert-Hash `
            ...
            -ExpectedHash $ExpectedOfficialCatalogHash `
            -Label 'Original Microsoft-signed AMD u0202082.cat'
    ...
    $CanonicalSigningState['Workflow'] = 'LegionGo-AMD-26.6.4'
    ...
    Write-Host ''
    Write-Host '=== VERIFY COMPATIBLE LEGION GO STARTING STATE ==='
    ...
    if (-not (Test-Path -LiteralPath $ActiveInfPathBefore -PathType Leaf)) {
        throw "Starting display INF is missing: $ActiveInfPathBefore"
    }
    ...
    if (
        [string]$GpuEntityBefore.Status -ne 'OK' -or
        [int]$GpuEntityBefore.ConfigManagerErrorCode -ne 0
    ) {
        throw (
            'Starting GPU is unhealthy: status ' +
            [string]$GpuEntityBefore.Status +
            ', code ' +
            [string]$GpuEntityBefore.ConfigManagerErrorCode
        )
    }

    $GpuEnumPathBefore =
        'HKLM:\SYSTEM\CurrentControlSet\Enum\' +
        [string]$GpuDriverBefore.DeviceID

    $GpuEnumValuesBefore =
        Get-ItemProperty -LiteralPath $GpuEnumPathBefore
    $KernelServiceNameBefore = [string]$GpuEnumValuesBefore.Service

    if ([string]::IsNullOrWhiteSpace($KernelServiceNameBefore)) {
        throw 'The starting GPU device has no kernel service assignment.'
    }

    $KernelServiceBefore =
        Get-CimInstance Win32_SystemDriver |
            Where-Object Name -EQ $KernelServiceNameBefore |
            Select-Object -First 1

    if ($null -eq $KernelServiceBefore) {
        throw (
            'Starting GPU kernel service was not found: ' +
            $KernelServiceNameBefore
        )
    }

    $KernelPathBefore =
        Resolve-KernelPath -RawPath $KernelServiceBefore.PathName

    if (-not (Test-Path -LiteralPath $KernelPathBefore -PathType Leaf)) {
        throw "Starting GPU kernel file is missing: $KernelPathBefore"
    }

    if ([IO.Path]::GetFileName($KernelPathBefore) -ine 'amdkmdag.sys') {
        throw "Unexpected starting GPU kernel file: $KernelPathBefore"
    }

    if (
        [string]$KernelServiceBefore.State -ne 'Running' -or
        -not [bool]$KernelServiceBefore.Started
    ) {
        throw (
    ...
    $KernelHashBefore = Get-SHA256 -LiteralPath $KernelPathBefore
    $DriverIsSignedBefore = [bool]$GpuDriverBefore.IsSigned
    $DriverSignerBefore = [string]$GpuDriverBefore.Signer
    $KernelSignatureBefore = Get-AuthenticodeSignature -LiteralPath $KernelPathBefore
    $KernelSignatureBeforeValid =
        [string]$KernelSignatureBefore.Status -eq 'Valid' -and
        $null -ne $KernelSignatureBefore.SignerCertificate
    $KernelSignerBefore = if ($KernelSignatureBeforeValid) {
        [string]$KernelSignatureBefore.SignerCertificate.Subject
    }
    else {
        ''
    }
    ...
    if (
        -not $DriverIsSignedBefore -or
        [string]::IsNullOrWhiteSpace($DriverSignerBefore)
    ) {
        if (-not $KernelSignatureBeforeValid) {
            throw (
    ...
    $StartingDisplayState = [ordered]@{
        DeviceName = [string]$GpuDriverBefore.DeviceName
        DeviceID = [string]$GpuDriverBefore.DeviceID
        Provider = [string]$GpuDriverBefore.DriverProviderName
        ActiveINF = [string]$GpuDriverBefore.InfName
        DriverVersion = [string]$GpuDriverBefore.DriverVersion
        ActiveInfSHA256 = $ActiveInfHashBefore
        Status = [string]$GpuEntityBefore.Status
        ProblemCode = [int]$GpuEntityBefore.ConfigManagerErrorCode
        KernelService = $KernelServiceNameBefore
        KernelPath = $KernelPathBefore
        KernelSHA256 = $KernelHashBefore
        DriverIsSigned = $DriverIsSignedBefore
        DriverSigner = $DriverSignerBefore
        KernelSignatureValid = $KernelSignatureBeforeValid
        KernelSigner = $KernelSignerBefore
    ...
    $TargetAlreadyActive =
        [string]$GpuDriverBefore.DriverVersion -eq $ExpectedTargetVersion -and
        $ActiveInfHashBefore -eq $ExpectedInfHash

    if (-not $TargetAlreadyActive) {
        $RecordedStartingState =
            Get-FirstPropertyValue `
                -Object $BootPreparation `
                -Names @('StartingDisplayState')

        if ($null -eq $RecordedStartingState) {
            throw (
                'Script 1 did not record StartingDisplayState. Rerun ' +
                'Script 1 from this Public Beta v2.0 release before ' +
                'continuing.'
            )
        }

        if (
            [string]$RecordedStartingState.DeviceID -ne
                [string]$StartingDisplayState.DeviceID -or
            [string]$RecordedStartingState.ActiveINF -ne
                [string]$StartingDisplayState.ActiveINF -or
            [string]$RecordedStartingState.DriverVersion -ne
                [string]$StartingDisplayState.DriverVersion -or
            [string]$RecordedStartingState.ActiveInfSHA256 -ne
                [string]$StartingDisplayState.ActiveInfSHA256 -or
            [string]$RecordedStartingState.KernelSHA256 -ne
                [string]$StartingDisplayState.KernelSHA256 -or
            [string]$RecordedStartingState.LenovoExtensionInfSHA256 -ne
                [string]$StartingDisplayState.LenovoExtensionInfSHA256 -or
            [string]$RecordedStartingState.LenovoExtensionCatalogSHA256 -ne
                [string]$StartingDisplayState.LenovoExtensionCatalogSHA256 -or
            [string]$RecordedStartingState.LenovoExtensionVersion -ne
                [string]$StartingDisplayState.LenovoExtensionVersion -or
            [string]$RecordedStartingState.LenovoCNVersion -ne
                [string]$StartingDisplayState.LenovoCNVersion -or
            [string]$RecordedStartingState.LenovoStableReleaseVersion -ne
                [string]$StartingDisplayState.LenovoStableReleaseVersion
        ) {
            throw (
                'The live starting display state changed after Script 1. ' +
                'Rerun Script 1 before installing the target driver.'
            )
        }

        Write-Host (
            '[PASS] Healthy compatible starting state matches the dynamic ' +
            'identity recorded by Script 1'
        )
    ...
    Write-Host ''
    Write-Host '=== STAGE CORRECTED 26.6.4 PACKAGE ==='
...
'@
    ...
    'Phase-02-Verify-Normal-Signing.ps1' = [ordered]@{
        SHA256 = 'FE82DA6480625136A3B3F9AFF75D3D989AC157614A5DE230DD9A7AE3ABF51611'
        Utf8Bom = $false
        LineEnding = 'LF'
        TrailingNewline = $true
        ContentParts = @(
...
<#
Phase 2 of Script 2 in the Legion Go AMD 26.6.4 Toolkit.
...
Actions:
  ...
  - Verify the corrected 26.6.4 driver survived the reboot.
...
param(
    [string]$InstallResultPath =
        'C:\ProgramData\LegionGo-AMD-26.6.4\driver-install-result.json',
...
function Get-ValidatedDriverStorePackage {
    $Matches = @(
        Get-ChildItem `
            ...
            -ErrorAction Stop |
            Where-Object {
                $InfPath = Join-Path $_.FullName 'u0202082.inf'
...
try {
    ...
    Assert-Condition `
        ...
        -Message 'Corrected 26.6.4 driver survived reboot' `
    ...
    $LocalCatalogPath = Join-Path $DriverStorePath 'u0202082.cat'
    ...
    Assert-Condition `
        -Condition $ExtensionState.Compatible `
        -Message 'Semantically compatible Lenovo extension remains attached' `
        -Details (
            "Matches=$($ExtensionState.MatchingRecords.Count); " +
            "Failures=" + (@($ExtensionState.FailureReasons) -join ' | ')
        )

    $ExtensionRecord = if ($ExtensionState.Compatible) {
        @($ExtensionState.MatchingRecords)[0]
    }
    else {
        $null
    }
    ...
    $SystemEvents = @(
        Get-WinEvent `
            ...
            Where-Object {
                ...
                $_.Message -match
                    '(?i)amdkmdag|u0202082|VEN_1002|DEV_15BF'
            }
    )
    ...
    $ValidationResult = [ordered]@{
        ...
        LenovoExtensionSemanticCompatible = $ExtensionState.Compatible
        LenovoExtensionOriginalName = [string]$ExtensionRecord.OriginalName
        LenovoExtensionPublishedName = [string]$ExtensionRecord.PublishedName
        LenovoExtensionVersion = [string]$ExtensionRecord.Version
        LenovoExtensionClassGuid = [string]$ExtensionRecord.ClassGuid
        LenovoExtensionId = [string]$ExtensionRecord.ExtensionId
        ...
        LenovoExtensionCatalogSigner = [string]$ExtensionRecord.CatalogSigner
        LenovoCNVersion = [string]$ExtensionRecord.CNVersion
        LenovoCNDriverVersion = [string]$ExtensionRecord.CNDriverVersion
        LenovoStableReleaseVersion = [string]$ExtensionRecord.StableReleaseVersion
