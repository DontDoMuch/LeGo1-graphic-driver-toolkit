...
<##
Stage 2 performs the controlled RC2zk target-device binding:
...
function Write-Stage2Result {
    ...
    $Contract = [ordered]@{
        Schema='LegionGo-AMD-26.7.1-v3.0-RC2zk-Stage2Result'
        Release='v3.0-RC2zk'
...
function Invoke-BestEffortRollback($State,[string]$RecoveryStatus,[string]$Reason) {
    ...
    if ($PreviousOriginKind -eq 'MicrosoftBasic') {
        ...
        Write-Host '[INFO] RC2zk candidate remains staged after third-party rollback by design; active binding determines the live driver.'
    }

    if (-not [string]::IsNullOrWhiteSpace($RollbackExtensionInf)) {
        if (Test-Path -LiteralPath $RollbackExtensionInf -PathType Leaf) {
            try {
                $ExtensionResult = Invoke-Pnp -Arguments @('/add-driver',$RollbackExtensionInf,'/install') -Name 'rollback-restore-extension' -AllowFailure
                $ExtensionRestoreOk = ($ExtensionResult.ExitCode -eq 0)
            } catch { $ExtensionRestoreOk = $false }
        } else { $ExtensionRestoreOk = $false }
    }
    ...
    if (-not [string]::IsNullOrWhiteSpace($RollbackExtensionInf)) {
        if ($ExtensionRestoreOk) { Write-Host '[PASS] Lenovo extension restore' -ForegroundColor Green }
        else { Write-Host '[FAIL] Lenovo extension restore did not report success' -ForegroundColor Red }
    }
...
function Verify-NormalFinal($State) {
    ...
    if ($FinalRows.Count -lt 1) { throw 'No exact final RC2zk Display-class package remains staged.' }
    Complete-ActivityProgress -Activity 'Verify exact final Display package remains staged' -Detail ("exact RC2zk rows={0}" -f $FinalRows.Count)
    Write-Host ("[INFO] AMD Display-class packages staged: {0}; exact RC2zk rows: {1}. Inactive packages are retained by design." -f $Display.Count,$FinalRows.Count)
    Apply-SignerTrustPolicy -State $State
    return $Gpu
}

Write-StageStatus -Stage 2 -Name 'Driver binding' -CurrentTask 'Export rollback package, retain previous display package, bind RC2zk to Legion Go, verify recovery-safe result'
...
if ($ResumeInstalledCandidate) {
    ...
    if ($ResumeFinalRows.Count -lt 1) { throw 'Interrupted-session resume cannot find the exact final RC2zk package in Driver Store.' }
    $RollbackRoot = Get-StateTextProperty -State $State -Name 'RollbackRoot'
    ...
    if ($Extensions.Count -gt 1) { throw "Multiple amduw23e packages are staged; refusing an ambiguous transaction: $($Extensions.Count)" }
    $Extension = $null
    if ($Extensions.Count -eq 1) {
        $Extension = $Extensions[0]
        $ExtensionText = Get-Content -LiteralPath ([string]$Extension.OriginalFileName) -Raw -ErrorAction Stop
        if ($ExtensionText -notmatch 'VEN_1002&DEV_15BF&SUBSYS_381217AA') { throw 'The staged amduw23e package does not target the original Legion Go; refusing to remove it.' }
        Write-Host "Standalone Lenovo extension staged as: $($Extension.Driver)"
    } else { Write-Host '[INFO] No standalone amduw23e package is staged.' }
    ...
    if ($null -ne $Extension) {
        $ExtensionHash = Get-SHA256 ([string]$Extension.OriginalFileName)
        [void](Invoke-Pnp -Arguments @('/export-driver',([string]$Extension.Driver),$RollbackExt) -Name 'export-lenovo-extension')
        $RollbackExtensionInf = Get-ExportedInfByHash -Root $RollbackExt -ExpectedHash $ExtensionHash
    }
    if ($CurrentOriginKind -eq 'MicrosoftBasic') {
    ...
    if (-not [string]::IsNullOrWhiteSpace($RollbackExtensionInf)) { Write-Host "[PASS] Rollback extension: $RollbackExtensionInf" -ForegroundColor Green }
    ...
    Write-Host 'remove the standalone Lenovo extension when present, and explicitly bind RC2zk only to the original Legion Go GPU.'
    ...
    try {
        ...
        if ($StagedFinalRows.Count -lt 1) { throw 'The exact RC2zk display package is not present in Driver Store after staging.' }
        ...
        Write-Host ("[PASS] Exact RC2zk package staged. AMD Display inventory count={0}; inactive packages retained." -f $StagedDisplayRows.Count) -ForegroundColor Green

        if ($null -ne $Extension) {
            [void](Invoke-Pnp -Arguments @('/delete-driver',([string]$Extension.Driver),'/uninstall','/force') -Name 'remove-lenovo-extension')
            if (@(Get-LenovoExtensionDrivers).Count -ne 0) { throw 'Standalone amduw23e remains staged after deletion.' }
        }
