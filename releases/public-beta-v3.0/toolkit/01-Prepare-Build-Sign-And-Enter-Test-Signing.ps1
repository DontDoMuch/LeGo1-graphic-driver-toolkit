...
<##
Legion Go 1 / AMD 26.7.1 v3.0 RC2zk validation candidate.
...
function Remove-OrphanPrivateSigners {
    $Prefix = 'CN=LegionGo AMD 26.7.1 v3.0 RC2zk Local Driver ' 
    $Removed = 0
    foreach ($Signer in @(Get-ChildItem -LiteralPath 'Cert:\LocalMachine\My' -ErrorAction SilentlyContinue)) {
        if ([string]$Signer.Subject -like ($Prefix + '*')) {
            try { Remove-Item -LiteralPath $Signer.PSPath -Force -ErrorAction Stop; $Removed++ } catch { throw "Unable to remove an orphaned RC2zk private signing certificate: $($_.Exception.Message)" }
        }
    }
    if ($Removed -gt 0) { Write-Host ("[RECOVERY] Removed {0} orphaned RC2zk private signing certificate(s) from LocalMachine\My." -f $Removed) -ForegroundColor Yellow }
}
...
$WorkRoot=Join-Path $env:SystemDrive ('AMD\LegionGo-AMD-26.7.1-v3.0-RC2zk-'+$Timestamp)
...
$Subject='CN=LegionGo AMD 26.7.1 v3.0 RC2zk Local Driver '+$Timestamp
$Cert=New-SelfSignedCertificate -Type CodeSigningCert -Subject $Subject -FriendlyName 'LegionGo AMD 26.7.1 v3.0 RC2zk Local Catalog Signing' -CertStoreLocation 'Cert:\LocalMachine\My' -KeyAlgorithm RSA -KeyLength 2048 -HashAlgorithm SHA256 -KeyExportPolicy NonExportable -NotAfter (Get-Date).AddYears(5)
...
$CerPath=Join-Path $WorkRoot 'LegionGo-AMD-26.7.1-v3.0-RC2zk-Public.cer'
