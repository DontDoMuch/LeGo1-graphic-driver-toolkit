...
display-driver INF. RC2zk never uses MSI repair mode for the target products:
...
function Install-FreshMsiVerified {
    ...
    if ((Get-MsiProductState -ProductCode $ProductCode) -eq 5) {
        throw "$Name is still registered before a fresh install. RC2zk never invokes target-product MSI repair mode."
    }
    $MsiArguments = @('/i',$Path)
...
try {
    ...
    if (-not $AlreadyHealthy) {
        ...
        Write-Host 'RC2zk NEVER invokes MSI repair mode and NEVER touches a display-driver INF in Stage 3.'
