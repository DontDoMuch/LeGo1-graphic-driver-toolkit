...
<##
One-command, resumable launcher for Legion Go AMD 26.7.1 Public Beta v3.1
...
function Invoke-PackagePreflight {
    ...
    $Preflight = Join-Path $ToolkitRoot 'Preflight-AMD-26.7.1-Public-Beta-v3.1.ps1'
    ...
    try {
        ...
        $ContractPass = ($null -ne $Contract -and [string]$Contract.Schema -eq 'LegionGo-PublicBeta-v3.1-PreflightResult-v1' -and [bool]$Contract.Passed -and [int]$Contract.FailedChecks -eq 0)
        ...
        if ($Code -ne 0 -or -not $ContractPass) {
            ...
            $FailedNames = ''
            if ($null -ne $Contract -and $null -ne $Contract.PSObject.Properties['FailedCheckNames']) {
                $FailedNames = (@($Contract.FailedCheckNames) -join ', ')
            }
            if (Test-Path -LiteralPath $ResultPath -PathType Leaf) {
                $PreservedPreflightResult = Join-Path $Logs ('FAILED-' + [IO.Path]::GetFileName($ResultPath))
                Copy-Item -LiteralPath $ResultPath -Destination $PreservedPreflightResult -Force
            }
            throw ("Preflight result contract failed. ProcessExitCode={0} Contract={1}; FailedNames={2}. No driver stage was started." -f $Code,$ContractText,$FailedNames)
        }
...
function Initialize-PersistentToolkit {
    ...
    if ($null -ne $ExistingState) {
        if ([string]$ExistingState.Release -ne $Release) { throw "Another workflow release is already present in this Public Beta v3.1 workflow root: $([string]$ExistingState.Release)" }
...
function Assert-OneClickSecureBootReady {
    ...
    if ($SecureBoot -eq $true) {
        ...
        throw 'Secure Boot is enabled. Disable it in UEFI firmware and rerun Public Beta v3.1.'
    }
    if ($null -eq $SecureBoot) {
        throw 'Secure Boot state could not be determined. Public Beta v3.1 fails closed until Secure Boot can be proven disabled.'
    }
...
function Request-OneClickConsent {
    ...
    if ($AutomaticResume) {
        throw 'Automatic resume found no valid Public Beta v3.1 one-click consent record. No stage will run unattended.'
    }
...
function Invoke-StageProcess {
    ...
    if ($IsStage2) {
        ...
        if ([string]$Stage2Result.Release -ne 'Public-Beta-v3.1' -or [string]$Stage2Result.InvocationId -ne $InvocationId) {
    ...
    if ($IsStage3) {
        ...
        if ([string]$Stage3Result.Release -ne 'Public-Beta-v3.1' -or [string]$Stage3Result.InvocationId -ne $InvocationId) {
    ...
    if ($IsStage4) {
        ...
        if ([string]$Stage4Result.Release -ne 'Public-Beta-v3.1' -or [string]$Stage4Result.InvocationId -ne $InvocationId) {
...
function Prepare-FinalEvidenceContracts {
    ...
    if ([string]$Stage2Contract.Release -ne 'Public-Beta-v3.1' -or [string]$Stage2Contract.Status -ne 'Passed' -or [int]$Stage2Contract.ExitCode -ne 0) {
        throw 'Stage 2 final result contract is not a clean Public Beta v3.1 PASS; refusing to package misleading final evidence.'
    }
    if ([string]$Stage3Contract.Release -ne 'Public-Beta-v3.1' -or [int]$Stage3Contract.Stage -ne 3 -or [string]$Stage3Contract.Status -ne 'Passed' -or [int]$Stage3Contract.ExitCode -ne 0) {
        throw 'Stage 3 final result contract is not a clean Public Beta v3.1 PASS; refusing to package misleading final evidence.'
    }
    if ([string]$Stage4Contract.Release -ne 'Public-Beta-v3.1' -or [string]$Stage4Contract.Status -ne 'Passed' -or [int]$Stage4Contract.ExitCode -ne 0 -or [int]$Stage4Contract.FailedChecks -ne 0) {
        throw 'Stage 4 final result contract is not a clean Public Beta v3.1 PASS; refusing to package misleading final evidence.'
    }
...
function New-FailureEvidenceBundle {
    ...
    $Downloads = Join-Path $env:USERPROFILE 'Downloads'
    $EvidencePath = Join-Path $Downloads ('LegionGo-AMD-26.7.1-Public-Beta-v3.1-Failure-Evidence-' + $Stamp)
    $ZipPath = $EvidencePath + '.zip'
    try {
        New-Item -ItemType Directory -Path $EvidencePath -Force | Out-Null
        $Summary = @(
            'LEGION GO AMD 26.7.1 PUBLIC BETA v3.1 - FAILURE EVIDENCE',
...
try {
    Write-Host '============================================================'
    Write-Host ' LEGION GO AMD 26.7.1 PUBLIC BETA v3.1 - ONE-COMMAND INSTALLER'
    Write-Host '============================================================'
