...
<##
Read-only package preflight for v3.0 RC2zp. This script reads release files,
...
Write-Host '========================================================================'
Write-Host 'LEGION GO AMD 26.7.1 PUBLIC BETA v3.1 - READ-ONLY PACKAGE PREFLIGHT'
Write-Host '========================================================================'
...
Check 'RC2ZP_ACTIVE_CATALOG_RESOLVED_FROM_INF' ($Common.Contains('function Get-CatalogFileNameFromInfLines') -and $Common.Contains('function Resolve-DriverStoreCatalogForPublishedInf') -and $S2.Contains('Resolve-DriverStoreCatalogForPublishedInf -PublishedInf ([string]$Gpu.ActiveINF)') -and -not $S2.Contains('Join-Path $StoreRoot ''u0202643.cat''')) 'Stage 2 resolves the active catalog declared by the active Driver Store INF instead of hardcoding the final AMD catalog name while inspecting OEM origins.'
Check 'RC2ZP_FINAL_AUDIT_CATALOG_RESOLVED_FROM_INF' ($S4.Contains('Resolve-DriverStoreCatalogForPublishedInf -PublishedInf $Gpu.ActiveINF') -and $S4.Contains('ACTIVE_CATALOG_NAME')) 'Final audit resolves the active catalog from the active INF and separately requires the final merged driver to declare u0202643.cat.'
Check 'RC2ZP_CATALOG_RESOLVER_FAIL_CLOSED' ($Common.Contains('Active Driver Store INF contains no applicable CatalogFile directive for x64 Windows.') -and $Common.Contains('Ambiguous CatalogFile directives for x64 Windows:') -and $Common.Contains('CatalogFile directive must name a catalog in the INF repository root.') -and $Common.Contains('Active display catalog declared by $InfPath is missing: $CatalogPath')) 'Catalog resolution fails closed on missing, ambiguous, non-root, non-CAT, or physically absent catalog declarations.'
Check 'RC2ZP_CATALOG_LINES_ALLOW_EMPTY_STRING' ($Common.Contains('[Parameter(Mandatory=$true)][AllowEmptyString()][string[]]$Lines')) 'Real INF files contain blank lines; the catalog resolver must allow empty-string elements from Get-Content under Windows PowerShell 5.1.'
...
try {
    ...
    Check 'RC2ZP_FIXTURE_OEM_GENERIC_CATALOG' ($CatalogGeneric -ieq 'u0198040.cat') 'A Lenovo-style generic CatalogFile directive resolves to its OEM catalog instead of u0202643.cat.'
    ...
    Check 'RC2ZP_FIXTURE_REAL_INF_BLANK_LINES_ACCEPTED' ($CatalogBlankLines -ieq 'u0198040.cat') 'A real-INF-shaped line array containing blank lines and comments must bind and resolve normally under Windows PowerShell 5.1.'
    ...
    Check 'RC2ZP_FIXTURE_X64_DECORATED_CATALOG_WINS' ($CatalogX64 -ieq 'x64.cat') 'CatalogFile.NTamd64 is authoritative over generic and incompatible-architecture declarations on x64 Windows.'
    ...
    Check 'RC2ZP_FIXTURE_AMBIGUOUS_CATALOG_REJECTED' $AmbiguousRejected 'Conflicting equally authoritative x64 CatalogFile declarations are rejected rather than guessed.'
    ...
    Check 'RC2ZP_FIXTURE_CATALOG_PATH_TRAVERSAL_REJECTED' $TraversalRejected 'CatalogFile path traversal/subdirectory values are rejected; only a catalog in the active INF repository root is accepted.'
}
catch {
    Check 'RC2ZP_CATALOG_RESOLVER_FIXTURE_EXECUTION' $false $_.Exception.Message
}

Check 'RC2ZP_FINAL_DRIVER_DAT_PATH_LITERAL' ($Common.Contains("Join-Path `$Root 'B026283\amdgcf.dat'") -and $Common.Contains("'active B026283\amdgcf.dat'")) 'Assert-FinalDriver uses a literal backslash path for B026283\amdgcf.dat and cannot contain the RC2zd BEL-character regression.'
Check 'RC2ZP_STAGE2_RECOVERY_EXITS_NONZERO' ($S2.Contains("Write-FailureGuide -Stage 'Stage 2 - driver replacement/recovery'") -and $S2.Contains("Set-StateProperty `$State 'RecoveryReason' `$Reason") -and $S2.Contains('        exit 1')) 'A destructive Stage 2 exception performs recovery and then exits nonzero explicitly instead of relying on a rethrow/process-exit interaction.'
Check 'RC2ZP_FAILURE_EVIDENCE_BUNDLE' ($Launcher.Contains('function New-FailureEvidenceBundle') -and $Launcher.Contains('Public-Beta-v3.1-Failure-Evidence-') -and $Launcher.Contains('Failure-Summary.txt') -and $Launcher.Contains('[void](New-FailureEvidenceBundle -Reason $LauncherFailure -LauncherLog $TranscriptPath)')) 'Hard launcher failures create a best-effort Downloads failure-evidence folder and ZIP even when Stage 4 was never reached.'
...
Check 'NO_DDU_AUTOMATION' (-not($AllText-match '(?i)display\s+driver\s+uninstaller|\bddU\b')) 'DDU support is intentionally deferred; RC2zp hardens portable-tool identity, scoped QuickEdit behavior, and reboot-resume orchestration only.'
...
Check 'RC2ZP_ONE_CLICK_CONSENT_PERSISTENCE' ($Common.Contains('$Script:InstallConsentPath = Join-Path $Script:WorkflowRoot ''one-click-consent.json''') -and $Common.Contains('AutomaticRebootsApproved=$true') -and $Common.Contains('Save-OneClickConsent') -and $Common.Contains('Remove-OneClickConsent')) 'The two initial confirmations are persisted only for the resumable RC2zp workflow and can be revoked on failure/completion.'
Check 'RC2ZP_SECURE_BOOT_FRONT_GATE' ($Launcher.Contains('Assert-OneClickSecureBootReady') -and $Launcher.Contains('[BLOCKED] Secure Boot is currently ENABLED.') -and $Launcher.Contains('UEFI Firmware Settings') -and $S1.Contains('Secure Boot is enabled') -and $S2.Contains('$SecureBootNow=Get-SecureBootState') -and $S4.Contains('SECURE_BOOT_DISABLED_FOR_LOCAL_SIGNER')) 'Secure Boot is checked before managed consent, rechecked before the driver transaction, and audited at the end; enabled/unknown states fail closed.'
Check 'RC2ZP_LITERAL_PROGRESS_FRAMEWORK' ($Common.Contains('function Get-ConsoleProgressBar') -and $Common.Contains('function Write-ActivityProgress') -and $Common.Contains('function Write-IndeterminateActivityProgress') -and $Common.Contains('[TASK]     ') -and $Launcher.Contains('Read-only package/parser preflight') -and $S1.Contains('190 AMD-unchanged files') -and $S2.Contains('Stage 2 frozen 190-file guard') -and $S3.Contains('Verify seven AMD runtime files') -and $S3.Contains('Verify AMD runtime SHA-256: ') -and $S4.Contains('Final audit runtime SHA-256: ') -and $S4.Contains('$ExpectedAuditCheckCount=64')) 'Every major logical step has a task-local progress lifecycle; determinate work uses real percentages and indeterminate Windows queries use a moving activity bar.'
Check 'RC2ZP_OFFICIAL_INSTALLER_HASH_PROGRESS' ($Common.Contains('function Get-SHA256WithProgress') -and $Common.Contains("-Activity 'Verify official AMD 26.7.1 installer SHA-256'")) 'The ~890 MB official installer hash no longer creates an unreported white-time interval.'
Check 'RC2ZP_BACKGROUND_LIVENESS_WATCHDOG' ($Common.Contains('public static class LegionGoConsoleLivenessRC2zp') -and $Common.Contains('new Timer(Tick, null, 500, 500)') -and $Common.Contains('[LIVE]     ') -and $Common.Contains('STILL WORKING') -and $Common.Contains('Configure(2000,2000)')) 'A separate .NET ThreadPool timer can repaint liveness every ~2 seconds even while the PowerShell runspace is blocked in a synchronous call.'
Check 'RC2ZP_LIVENESS_DOES_NOT_FAKE_PERCENT' ($Common.Contains('last checkpoint: ') -and $Common.Contains('current: ') -and $Common.Contains('Percent = Math.Max(0, Math.Min(100, percent))') -and $Common.Contains('Set-ActivityLivenessContext')) 'The watchdog repeats the last real percentage and elapsed time; it does not advance determinate percentage based on wall-clock time.'
Check 'RC2ZP_190_FILE_CURRENT_ITEM_LIVENESS' ($S1.Contains('Set-ActivityLivenessContext -Activity $Label') -and $S1.Contains('file {0}/{1}: {2} ({3:N1} MB)') -and $S2.Contains("Set-ActivityLivenessContext -Activity 'Stage 2 frozen 190-file guard'")) 'Long 190-file guards expose the exact file/size currently being hashed while the outer real percentage remains authoritative.'
Check 'RC2ZP_POST_BIND_LIVENESS' ($Common.Contains('$BindActivity=''Explicit Legion Go GPU bind''') -and $Common.Contains('Set-ActivityLivenessContext -Activity $BindActivity -Status ''BINDING''') -and $S2.Contains('Allow PnP state to settle after GPU bind') -and $S2.Contains('Query live Legion Go GPU binding after bind') -and $S2.Contains('Verify active Driver Store identities after GPU bind') -and $S2.Contains('Resolve and verify active Driver Store catalog') -and $S2.Contains('Enumerate AMD Display Driver Store inventory after bind')) 'The post-bind path has explicit task-local liveness for the blocking native bind, PnP settle countdown, GPU rediscovery, active Driver Store validation, catalog verification, and inventory.'
Check 'RC2ZP_FINAL_AUDIT_LIVENESS_CONTEXT' ($S4.Contains("Set-ActivityLivenessContext -Activity 'Final persistence audit'") -and $S4.Contains('Windows Authenticode validation for active display catalog') -and $S4.Contains('enumerate AMD Display packages with Get-WindowsDriver -Online -All')) 'Final audit keeps a continuously updating elapsed/liveness signal between completed percentage checkpoints and names its slow catalog/inventory sub-operations.'
Check 'RC2ZP_OVERALL_SEPARATE_FROM_TASK_LIVENESS' ($Common.Contains('[OVERALL] Stage {0}/4 :: {1}') -and $Common.Contains('[TASK]     ') -and -not $Common.Contains("Write-ActivityProgress -Activity ('Overall workflow - Stage '") -and -not $Common.Contains('$Script:CurrentSectionActivity')) 'Overall stage position is informational only and never becomes the task/liveness percentage.'
Check 'RC2ZP_SECTION_HEADINGS_NOT_PROGRESS_TASKS' ($Common.Contains('$Script:CurrentSectionTitle') -and $Common.Contains('[SECTION PASS]') -and -not $Common.Contains('$Script:CurrentSectionActivity')) 'Section headings no longer create synthetic 0% tasks that can be repeated by the watchdog.'
Check 'RC2ZP_ACTIVE_CATALOG_LOCAL_PHASES' ($S2.Contains('Write-ActivityProgress -Activity $CatalogActivity -Percent 20 -Status ''RESOLVING''') -and $S2.Contains('Write-ActivityProgress -Activity $CatalogActivity -Percent 50 -Status ''HASHING''') -and $S2.Contains('Write-ActivityProgress -Activity $CatalogActivity -Percent 75 -Status ''SIGNATURE''') -and $S2.Contains('Assert-ExpectedActiveCatalog -State $State -Gpu $Post -Identity $ActiveCatalog')) 'Active-catalog work now shows visible local phase progress and reuses the already-proven identity instead of immediately repeating the same catalog query.'
Check 'RC2ZP_DRIVERSTORE_QUERIES_INDETERMINATE' ($S2.Contains("Write-IndeterminateActivityProgress -Activity 'Enumerate AMD Display Driver Store inventory after bind'") -and $S2.Contains("Write-IndeterminateActivityProgress -Activity 'Enumerate AMD Display Driver Store after staging candidate'") -and $S2.Contains("Write-IndeterminateActivityProgress -Activity 'Query current Legion Go GPU state before driver transaction'")) 'Windows/Driver Store operations without a measurable completion fraction are presented as ACTIVE rather than fake 0-to-100 percentages.'
Check 'RC2ZP_ASSERT_FINAL_DRIVER_LOCAL_PROGRESS' ($Common.Contains('Write-ActivityProgress -Activity $Activity -Percent 40 -Status ''RESOLVING''') -and $Common.Contains('Write-ActivityProgress -Activity $Activity -Percent 70 -Status ''RESOLVED''') -and $Common.Contains('Write-ActivityProgress -Activity $Activity -Percent 80 -Status ''HASHING''')) 'Exact-driver validation exposes its own Driver Store resolve/DAT-hash phases instead of leaving stale prior-task liveness on screen.'
Check 'RC2ZP_STAGE3_LOCAL_SOFTWARE_SEAL_PROGRESS' ($S3.Contains("Verify AMD Settings MSI registration") -and $S3.Contains("Verify AMD DVR MSI registration") -and $S3.Contains("Verify native RSXCM 22.10.0.0 registration") -and $S3.Contains("Verify AMD startup tasks") -and $S3.Contains("Verify legacy Radeon Store AppX is absent") -and $S3.Contains("Query Legion Go GPU for pre-reboot software seal")) 'AMD Software validation and the pre-reboot software seal are decomposed into task-local checks rather than inheriting a Stage 3 50% or section 0% heartbeat.'
...
Check 'ROLLBACK_STATUS_OUTPUT' ($S2.Contains('Previous display origin restore') -and $S2.Contains('Lenovo extension lineage restore') -and $S2.Contains('RollbackExtensionInfs') -and $S2.Contains('RecoveryGpuState')) 'Rollback reports origin-agnostic base restoration, multi-member extension-lineage restoration when applicable, and the observed recovery GPU state.'
...
Check 'ORPHAN_PRIVATE_SIGNER_CLEANUP' ($S1.Contains('Remove-OrphanPrivateSigners') -and $S1.Contains("Cert:\LocalMachine\My")) 'Stage 1 removes orphaned RC2zp private signing certificates left by an interrupted signing session.'
...
Check 'NO_FULL_SDK_WDK_INSTALL' (-not($Common.Contains("winget','install','--id','Microsoft.WindowsSDK.10.0.28000")) -and -not($Common.Contains("winget','install','--id','Microsoft.WindowsWDK.10.0.28000"))) 'RC2zp never installs the full Windows SDK/WDK packages.'
...
Check 'COMPANION_FIELD_CALIBRATION_CONTRACT' ($S1.Contains('Field calibration on 2026-08-19 proved all nine exact official') -and $S1.Contains('Microsoft Windows Hardware') -and $S1.Contains('SignTool /kp CAT=PASS; /kp INF membership=PASS')) 'RC2zp encodes the field-proven Microsoft WHCP result without weakening exact-hash or kernel-policy validation.'
...
Check 'FUTUREPROOF_ORIGIN_CLASSIFIER_PRESENT' ($Classifier.Contains("'LegacyStandaloneExtension'") -and $Classifier.Contains("'MergedEmbedded'") -and $Classifier.Contains("'MergedEmbeddedWithStaleExtension'") -and $Classifier.Contains("'ThirdPartyDisplay'")) 'RC2zp integrates the reusable future-proof origin architecture classifier.'
...
Check 'RC2ZP_ACTIVE_DRIVER_VERSION_PASSED_TO_CLASSIFIER' ($Common.Contains('DriverVersion=[string]$Gpu.DriverVersion') -and $Classifier.Contains('$ActiveDriverVersion = [string](Get-OptionalPropertyValue -InputObject $Gpu -Name ''DriverVersion'')') -and $Classifier.Contains('-ActiveDriverVersion $ActiveDriverVersion')) 'The live Legion Go display DriverVersion is explicitly passed into the reusable origin classifier.'
Check 'RC2ZP_EXTENSION_LINEAGE_AUTHORITY' ($Classifier.Contains('Select-LegionGoExtensionAuthority') -and $Classifier.Contains('Multiple staged amduw23e package generations share the same validated Go 1 Lenovo-derived ExtensionId') -and $Classifier.Contains('Multiple distinct standalone Lenovo amduw23e ExtensionId lineages are present') -and $Classifier.Contains('$VerifiedLenovoOemDisplayPass')) 'RC2zp classifies versioned Go 1 Lenovo-derived ExtensionId lineages instead of treating package count or semantic-marker identity as origin authority; distinct applicable lineages still fail closed.'
Check 'RC2ZP_CN_METADATA_SUPPLEMENTAL' ($Classifier.Contains('CN metadata is supplemental for origin classification.') -and $Classifier.Contains('CN metadata is supplemental and does not override the live device/lineage scope.')) 'Missing or stale CN DriverVersion metadata is supplemental provenance and cannot invalidate an otherwise structurally scoped Go 1 origin.'
Check 'RC2ZP_PUBLIC_2662_2664_PRIOR_TOOLKIT_ALLOWLIST' ($Common.Contains('39BD11386ABFE8CB964902B18159801A486AB22FCFA9C622622F4E6B9B9D901E') -and $Common.Contains('32.0.31021.1015') -and $Common.Contains('73E8AE95849354D3D52DCB2A583CCB458D33DF22ACCCD0F0C1EE7626FDBD3034') -and $Common.Contains('32.0.31021.5001') -and $Common.Contains('$VerifiedPriorToolkitDisplayPass') -and $Common.Contains('-VerifiedPriorToolkitDisplayPass:$VerifiedPriorToolkitDisplayPass')) 'The public 26.6.2 and field-proven public 26.6.4 toolkit bases are recognized only by exact active-INF SHA256 plus exact DriverVersion.'
Check 'RC2ZP_PRIOR_TOOLKIT_EXTENSION_ARCHITECTURE' ($Classifier.Contains("OriginKind = 'VerifiedPriorToolkitWithStandaloneExtension'") -and $Classifier.Contains("ExtensionDisposition = 'ExportThenRemoveForMergedTarget'") -and $Classifier.Contains('expected public 26.6.2/26.6.4 architecture') -and $Classifier.Contains('required standalone Lenovo amduw23e extension is absent')) 'Exact prior public toolkit bases require their attached Lenovo extension; the expected cross-version DriverVer mismatch is accepted only for this architecture, then the extension is exported before the 26.7.1 merged transition removes it.'
Check 'RC2ZP_STRUCTURALLY_SCOPED_GO1_ORIGIN' ($Classifier.Contains("OriginKind = 'ScopedGo1StandaloneExtension'") -and $Classifier.Contains('custom/unrecognized semantic contents') -and $Classifier.Contains('version mismatch is evidence, not proof of an invalid origin')) 'Structurally scoped AMD origins for the original Go 1 may transition even when their same-lineage extension has custom semantics or DriverVer; package scope, not Lenovo byte identity, is the destructive-action boundary.'
Check 'RC2ZP_MULTI_EXTENSION_TRANSACTION_SUPPORT' (-not $S2.Contains('Multiple amduw23e packages are staged; refusing an ambiguous transaction') -and $S2.Contains('all will be exported and removed') -and $S2.Contains('Set-StateProperty $State ''RollbackExtensionInfs''') -and $S2.Contains('remove-lenovo-extension-{0}')) 'Stage 2 exports, removes, and rollback-restores every validated member of one Lenovo ExtensionId lineage.'
Check 'RC2ZP_NO_EXTERNAL_PROJECT_SUPPORT_CLAIM' (-not $Classifier.Contains('third-party') -and -not $Classifier.Contains('other legitimate Go 1') -and $Classifier.Contains('not a compatibility/support claim for any external project')) 'RC2zp avoids claiming support or non-support for untested external Go 1 projects; it only defines this installer''s structural safety boundary.'
Check 'RC2ZP_EXTENSION_MEMBER_SCOPE_NOT_SEMANTIC_LOCKIN' ($Common.Contains('function Get-LenovoExtensionPackageEvidence') -and $S2.Contains('Go 1 extension lineage member structurally scoped') -and $S2.Contains('custom/unrecognized semantic contents') -and -not $S2.Contains('failed independent Lenovo semantic validation') -and $S2.Contains('PreviousStandaloneExtensionMembers')) 'Any number of same-lineage packages is accepted when each member is safely scoped by Extension class, exact Go 1 applicability, and the Lenovo-derived ExtensionId; semantic-marker differences are retained as evidence/warnings for interoperability.'
Check 'RC2ZP_EXACT_OEM_IDENTITY_PIN' ($Common.Contains('2F337E2DAD0FC1371203A846D9F0AB6EAA3FE056704956C38244D05E1E7ADB22') -and $Common.Contains('32.0.23017.1001') -and $Common.Contains('{07A2A561-D001-4503-B239-EF2FE0379EFB}')) 'The exact Lenovo OEM base and Lenovo ExtensionId lineage are pinned for the OEM mismatch exception.'
Check 'RC2ZP_STAGE2_FAILED_CONTRACT_EXIT_NORMALIZATION' ($Launcher.Contains('$Code -eq 0 -and $Stage2ContractCode -eq 1 -and $Stage2ContractStatus -eq ''Failed''') -and $Launcher.Contains('Treating the child failure contract as authoritative.') -and $Launcher.Contains('$Stage2EffectiveCode = 1')) 'A fresh invocation-bound Stage-2 Failed/1 contract is authoritative over the field-observed process-exit-0 transport anomaly; non-failure mismatches remain fail-closed.'
Check 'RC2ZP_STAGE2_FAILURE_DETAIL_PROPAGATION' ($Launcher.Contains('$FailureContractPath = Join-Path $Script:WorkflowRoot ''stage2-result.json''') -and $Launcher.Contains('$ChildFailureDetail = [string]$FailureContract.Detail') -and $Launcher.Contains('Stage failed with exit code {0}: {1} :: {2}')) 'An authoritative Stage-2 Failed contract propagates its exact child detail into launcher failure evidence instead of collapsing to a generic stage-exit message.'
...
try {
    ...
    $FixtureExt=[pscustomobject]@{
        ...
        Attached=$true
        DriverVersion='32.0.23017.1001'
        DriverDateSort='20260108'
        PublishedInf='oem15.inf'
    ...
    $FixtureBadExt=[pscustomobject]@{
        ...
        DriverDateSort='20260108'
        PublishedInf='oem99.inf'
        ExtensionId='{07A2A561-D001-4503-B239-EF2FE0379EFB}'
        ClassName='Extension'
    }

    $OemNoCn=Resolve-LegionGoAmdOrigin -Gpu $FixtureGpu -CnMetadata $FixtureCnAbsent -LenovoExtensions $FixtureExtensions -EmbeddedLenovoSemanticsPass:$false -AllowStructurallyScopedGo1Origin:$true
    Check 'RC2ZP_FIXTURE_CLEAN_OEM_CN_ABSENT_ACCEPTED' ([bool]$OemNoCn.OriginAcceptable -and [string]$OemNoCn.OriginKind -eq 'LegacyStandaloneExtension' -and [string]$OemNoCn.ExtensionDisposition -eq 'ExportThenRemove' -and [bool]$OemNoCn.StandaloneExtensionVersionCoherent) 'Exact clean-OEM shape with CN DriverVersion absent classifies LegacyStandaloneExtension / ExportThenRemove.'

    $OemStaleCn=Resolve-LegionGoAmdOrigin -Gpu $FixtureGpu -CnMetadata $FixtureCnMismatch -LenovoExtensions $FixtureExtensions -EmbeddedLenovoSemanticsPass:$false -AllowStructurallyScopedGo1Origin:$true
    Check 'RC2ZP_FIXTURE_STALE_CN_LIVE_COHERENT_ACCEPTED' ([bool]$OemStaleCn.OriginAcceptable -and [string]$OemStaleCn.OriginKind -eq 'LegacyStandaloneExtension' -and [bool]$OemStaleCn.StandaloneExtensionVersionCoherent -and (@($OemStaleCn.Warnings) -join ' ') -match 'supplemental') 'Stale CN metadata warns but does not override a matching live OEM display+extension pair.'

    $FixtureBadExtensions=@($FixtureBadExt)

    $ExactOemMismatch=Resolve-LegionGoAmdOrigin -Gpu $FixtureGpu -CnMetadata $FixtureCnAbsent -LenovoExtensions $FixtureBadExtensions -EmbeddedLenovoSemanticsPass:$false -VerifiedLenovoOemDisplayPass:$true -AllowStructurallyScopedGo1Origin:$true
    Check 'RC2ZP_FIXTURE_EXACT_OEM_VERSION_MISMATCH_ACCEPTED' ([bool]$ExactOemMismatch.OriginAcceptable -and [string]$ExactOemMismatch.OriginKind -eq 'LegacyStandaloneExtension' -and -not [bool]$ExactOemMismatch.StandaloneExtensionVersionCoherent -and (@($ExactOemMismatch.Warnings) -join ' ') -match 'exact frozen Lenovo OEM base') 'Exact frozen Lenovo OEM display plus a validated same-lineage Lenovo extension may carry a different DriverVer; the mismatch becomes an evidence warning rather than a false origin failure.'

    $UnknownOemMismatch=Resolve-LegionGoAmdOrigin -Gpu $FixtureGpu -CnMetadata $FixtureCnAbsent -LenovoExtensions $FixtureBadExtensions -EmbeddedLenovoSemanticsPass:$false -VerifiedLenovoOemDisplayPass:$false -AllowStructurallyScopedGo1Origin:$true
    Check 'RC2ZP_FIXTURE_SCOPED_GO1_VERSION_MISMATCH_ACCEPTED' ([bool]$UnknownOemMismatch.OriginAcceptable -and [string]$UnknownOemMismatch.OriginKind -eq 'ScopedGo1StandaloneExtension' -and -not [bool]$UnknownOemMismatch.StandaloneExtensionVersionCoherent -and (@($UnknownOemMismatch.Warnings) -join ' ') -match 'structurally scoped Go 1 origin') 'A healthy AMD origin with the exact Go 1 extension lineage is transition-compatible even when another project uses a different DriverVer.'

    $FixtureOldExt=[pscustomobject]@{
        TargetsLegionGo=$true
        SemanticCompatible=$true
        Attached=$true
        DriverVersion='31.0.24028.1001'
        DriverDateSort='20240422'
        PublishedInf='oem8.inf'
        ExtensionId='{07A2A561-D001-4503-B239-EF2FE0379EFB}'
        ClassName='Extension'
    }
    $DuplicateLineage=Resolve-LegionGoAmdOrigin -Gpu $FixtureGpu -CnMetadata $FixtureCnAbsent -LenovoExtensions @($FixtureOldExt,$FixtureExt) -EmbeddedLenovoSemanticsPass:$false -VerifiedLenovoOemDisplayPass:$true -AllowStructurallyScopedGo1Origin:$true
    Check 'RC2ZP_FIXTURE_DUPLICATE_SAME_LINEAGE_ACCEPTED' ([bool]$DuplicateLineage.OriginAcceptable -and [string]$DuplicateLineage.OriginKind -eq 'LegacyStandaloneExtension' -and [bool]$DuplicateLineage.StandaloneExtensionVersionCoherent -and (@($DuplicateLineage.Warnings) -join ' ') -match 'one versioned lineage') 'The reproduced DDU -> current Lenovo OEM state with old+new amduw23e generations under one ExtensionId is accepted and the active-version member is authoritative.'

    $FixtureForeignLineage=[pscustomobject]@{
        TargetsLegionGo=$true
        SemanticCompatible=$true
        Attached=$true
        DriverVersion='32.0.23017.1001'
        DriverDateSort='20260108'
        PublishedInf='oem88.inf'
        ExtensionId='{11111111-2222-3333-4444-555555555555}'
        ClassName='Extension'
    }
    $DistinctLineages=Resolve-LegionGoAmdOrigin -Gpu $FixtureGpu -CnMetadata $FixtureCnAbsent -LenovoExtensions @($FixtureExt,$FixtureForeignLineage) -EmbeddedLenovoSemanticsPass:$false -VerifiedLenovoOemDisplayPass:$true -AllowStructurallyScopedGo1Origin:$true
    Check 'RC2ZP_FIXTURE_DISTINCT_LINEAGES_REJECTED' (-not [bool]$DistinctLineages.OriginAcceptable -and [string]$DistinctLineages.OriginKind -eq 'InvalidAmbiguousExtension') 'Multiple distinct ExtensionId lineages remain fail-closed because Windows may apply one extension from each lineage.'

    $FixtureOlderExt=[pscustomobject]@{
        TargetsLegionGo=$true
        SemanticCompatible=$true
        Attached=$true
        DriverVersion='30.0.14000.1000'
        DriverDateSort='20220101'
        PublishedInf='oem3.inf'
        ExtensionId='{07A2A561-D001-4503-B239-EF2FE0379EFB}'
        ClassName='Extension'
    }
    $ThreeMemberLineage=Resolve-LegionGoAmdOrigin -Gpu $FixtureGpu -CnMetadata $FixtureCnAbsent -LenovoExtensions @($FixtureOlderExt,$FixtureOldExt,$FixtureExt) -EmbeddedLenovoSemanticsPass:$false -VerifiedLenovoOemDisplayPass:$true -AllowStructurallyScopedGo1Origin:$true
    Check 'RC2ZP_FIXTURE_THREE_MEMBER_SAME_LINEAGE_ACCEPTED' ([bool]$ThreeMemberLineage.OriginAcceptable -and [string]$ThreeMemberLineage.OriginKind -eq 'LegacyStandaloneExtension' -and [bool]$ThreeMemberLineage.StandaloneExtensionVersionCoherent) 'The classifier has no Count==2 special case; three legitimate generations in the exact same Lenovo ExtensionId lineage classify using the active-version member.'

    $FixtureCustomExt=[pscustomobject]@{
        TargetsLegionGo=$true
        SemanticCompatible=$false
        Attached=$true
        DriverVersion='33.7.7777.7'
        DriverDateSort='20260707'
        PublishedInf='oem123.inf'
        ExtensionId='{07A2A561-D001-4503-B239-EF2FE0379EFB}'
        ClassName='Extension'
    }
    $CustomProjectOrigin=Resolve-LegionGoAmdOrigin -Gpu $FixtureGpu -CnMetadata $FixtureCnAbsent -LenovoExtensions @($FixtureCustomExt) -EmbeddedLenovoSemanticsPass:$false -VerifiedLenovoOemDisplayPass:$false -AllowStructurallyScopedGo1Origin:$true
    Check 'RC2ZP_FIXTURE_NONFROZEN_GO1_LINEAGE_ACCEPTED' ([bool]$CustomProjectOrigin.OriginAcceptable -and [string]$CustomProjectOrigin.OriginKind -eq 'ScopedGo1StandaloneExtension' -and -not [bool]$CustomProjectOrigin.StandaloneExtensionSemanticPass -and (@($CustomProjectOrigin.Warnings) -join ' ') -match 'custom/unrecognized semantic contents') 'A non-frozen Go 1 lineage may carry different semantics/DriverVer without being falsely rejected when the package remains structurally scoped to the exact Go 1 Lenovo-derived ExtensionId.'
    ...
    $Prior2662=Resolve-LegionGoAmdOrigin -Gpu $Fixture2662Gpu -CnMetadata $FixtureCnAbsent -LenovoExtensions $FixturePriorToolkitExtensions -EmbeddedLenovoSemanticsPass:$false -VerifiedPriorToolkitDisplayPass:$true -AllowStructurallyScopedGo1Origin:$true
    Check 'RC2ZP_FIXTURE_PUBLIC_2662_STANDALONE_EXTENSION_ACCEPTED' ([bool]$Prior2662.OriginAcceptable -and [string]$Prior2662.OriginKind -eq 'VerifiedPriorToolkitWithStandaloneExtension' -and [string]$Prior2662.ExtensionDisposition -eq 'ExportThenRemoveForMergedTarget' -and -not [bool]$Prior2662.StandaloneExtensionVersionCoherent) 'Exact public 26.6.2 plus its intentionally retained Lenovo OEM-generation amduw23e classifies as a verified prior toolkit architecture.'

    $Prior2664=Resolve-LegionGoAmdOrigin -Gpu $Fixture2664Gpu -CnMetadata $FixtureCnAbsent -LenovoExtensions $FixturePriorToolkitExtensions -EmbeddedLenovoSemanticsPass:$false -VerifiedPriorToolkitDisplayPass:$true -AllowStructurallyScopedGo1Origin:$true
    Check 'RC2ZP_FIXTURE_PUBLIC_2664_STANDALONE_EXTENSION_ACCEPTED' ([bool]$Prior2664.OriginAcceptable -and [string]$Prior2664.OriginKind -eq 'VerifiedPriorToolkitWithStandaloneExtension' -and [string]$Prior2664.ExtensionDisposition -eq 'ExportThenRemoveForMergedTarget' -and -not [bool]$Prior2664.StandaloneExtensionVersionCoherent) 'Exact public 26.6.4 plus its intentionally retained Lenovo OEM-generation amduw23e classifies as a verified prior toolkit architecture.'

    $PriorMissingExt=Resolve-LegionGoAmdOrigin -Gpu $Fixture2664Gpu -CnMetadata $FixtureCnAbsent -LenovoExtensions @() -EmbeddedLenovoSemanticsPass:$false -VerifiedPriorToolkitDisplayPass:$true -AllowStructurallyScopedGo1Origin:$true
    Check 'RC2ZP_FIXTURE_PRIOR_TOOLKIT_MISSING_EXTENSION_REJECTED' (-not [bool]$PriorMissingExt.OriginAcceptable -and [string]$PriorMissingExt.OriginKind -eq 'InvalidPriorToolkitMissingExtension') 'An exact 26.6.2/26.6.4 toolkit base without its required standalone Lenovo extension is rejected rather than misclassified as a complete prior toolkit architecture.'

    $UnknownToolkitMismatch=Resolve-LegionGoAmdOrigin -Gpu $Fixture2664Gpu -CnMetadata $FixtureCnAbsent -LenovoExtensions $FixturePriorToolkitExtensions -EmbeddedLenovoSemanticsPass:$false -VerifiedPriorToolkitDisplayPass:$false -AllowStructurallyScopedGo1Origin:$true
    Check 'RC2ZP_FIXTURE_SCOPED_AMD_GO1_LINEAGE_ACCEPTED' ([bool]$UnknownToolkitMismatch.OriginAcceptable -and [string]$UnknownToolkitMismatch.OriginKind -eq 'ScopedGo1StandaloneExtension') 'A healthy non-frozen AMD Go 1 origin is not rejected solely for being non-frozen when their standalone extension is safely scoped to the exact Lenovo-derived Go 1 lineage.'
}
catch {
    Check 'RC2ZP_ORIGIN_FIXTURE_EXECUTION' $false $_.Exception.Message
}

Check 'RC2ZP_EXPLICIT_FORCE_BIND' ($Common.Contains('UpdateDriverForPlugAndPlayDevicesW') -and $Common.Contains('$Script:LegionGoHardwareId') -and $S2.Contains('Invoke-ForceBindLegionGoDriver -InfPath $CandidateInf')) 'RC2zp explicitly binds the exact candidate only to the original Legion Go hardware ID.'
Check 'RC2ZP_STAGE_BEFORE_BIND' ($S2.IndexOf("'stage-final-v2b'") -lt $S2.IndexOf('Invoke-ForceBindLegionGoDriver -InfPath $CandidateInf')) 'RC2zp stages the candidate before changing the live Legion Go binding.'
Check 'RC2ZP_RETAINS_PREVIOUS_DISPLAY' (-not $S2.Contains('''/delete-driver'',$CurrentPublishedInf') -and $S2.Contains('Previous display package retained in Driver Store')) 'The prior display package is retained as rollback material instead of deleted.'
Check 'RC2ZP_CURRENT_PUBLISHED_INF_PROBE_LITERAL_SAFE' (-not($PreflightSelf.Contains('Contains("''/delete-driver'',$CurrentPublishedInf")'))) 'The RC2r CurrentPublishedInf preflight regression is encoded as a non-interpolating literal in RC2zp.'

Check 'RC2ZP_NO_GLOBAL_DISPLAY_COUNT_GATE' (-not $S2.Contains('Post-transaction AMD Display package count is not exactly one') -and -not $S2.Contains('Expected exactly one AMD Display-class package')) 'Inactive AMD Display packages do not fail the target-device transaction.'
Check 'RC2ZP_FINAL_AUDIT_INVENTORY_ONLY' ((Get-Content -LiteralPath (Join-Path $Root '04-Final-Persistence-Audit.ps1') -Raw).Contains('AMD_DISPLAY_PACKAGE_INVENTORY') -and -not (Get-Content -LiteralPath (Join-Path $Root '04-Final-Persistence-Audit.ps1') -Raw).Contains('ONE_AMD_DISPLAY_PACKAGE')) 'Final audit records other AMD Display packages as inventory while the exact active Legion Go binding remains the hard contract.'
Check 'RC2ZP_ROLLBACK_REBIND' ($S2.Contains('PreviousOriginalInfPath') -and $S2.Contains('rollback-restage-previous-display') -and $S2.Contains('Invoke-ForceBindLegionGoDriver -InfPath $RestoreInf')) 'Third-party rollback force-binds the verified previous display INF without deleting the RC2zp candidate.'
Check 'RC2ZP_BASIC_ONLY_CANDIDATE_DELETE' ($S2.Contains('rollback-remove-candidate-basic-origin') -and $S2.Contains("PreviousOriginKind -eq 'MicrosoftBasic'")) 'Candidate deletion is reserved for Microsoft Basic rollback, where removal+rescan is the restoration mechanism.'
Check 'RC2ZP_HASPROBLEM_HEALTH' ($Common.Contains('HasProblem=') -and $Common.Contains('[bool]$Gpu.HasProblem')) 'Final GPU health requires Status OK, ProblemCode 0, and HasProblem false.'
...
Check 'RC2ZP_NO_TARGET_MSI_REPAIR_MODE' (-not $S3.Contains('REINSTALL=ALL') -and -not $S3.Contains('REINSTALLMODE=') -and $S3.Contains('RC2zp never invokes target-product MSI repair mode')) 'Stage 3 never repairs the exact target MSI products.'
Check 'RC2ZP_DIRTY_TARGET_FULL_UNINSTALL' ($S3.Contains('Remove-MsiProductVerified -ProductCode $ExpectedDvrProductCode -Name ''remove-dirty-target-dvr''') -and $S3.Contains('remains installed-local after full uninstall; refusing to continue')) 'A dirty exact DVR registration is fully removed and proved absent before fresh install.'
Check 'RC2ZP_FRESH_DVR_CIM_INSTALL' ($S3.Contains('Install-FreshMsiVerified -Path $Dvr -ProductCode $ExpectedDvrProductCode -ExpectedVersion $ExpectedDvrVersion -Name ''fresh-install-target-dvr'' -LaunchedFromCim') -and $S3.Contains('$MsiArguments += ''LAUNCHED_FROM_CIM=1''')) 'Fresh exact DVR install uses AMD LAUNCHED_FROM_CIM=1.'
Check 'RC2ZP_FRESH_DVR_CIM_PROBE_CONSISTENT' ($S3.Contains('$MsiArguments = @(''/i'',$Path)') -and $S3.Contains('if ($LaunchedFromCim) { $MsiArguments += ''LAUNCHED_FROM_CIM=1'' }') -and -not $S3.Contains('$Args += ''LAUNCHED_FROM_CIM=1''')) 'Stage 3 and its preflight agree on the non-automatic MSI argument variable used for the DVR CIM property.'

Check 'RC2ZP_VERBOSE_MSI_RESULT_CONTRACT' ($S3.Contains('@(''/L*v'',$VerboseLog)') -and $S3.Contains('MainEngineThread is returning') -and $S3.Contains('MSI engine result')) 'Every Stage 3 MSI operation emits verbose logging and cross-checks the MSI engine result against the process exit code.'
Check 'RC2ZP_PRE_REBOOT_SOFTWARE_SEAL' ($S3.Contains('Write-Section ''PRE-REBOOT SOFTWARE SEAL''') -and $S3.IndexOf('[void](Assert-InstalledSoftware)') -lt $S3.IndexOf('Set-StateProperty $State ''Stage'' ''AwaitingSoftwareReboot''')) 'The complete software/runtime seal runs before the reboot checkpoint is written.'
Check 'RC2ZP_HEALTHY_TARGET_RETAINED' ($S3.Contains('target MSIs will be retained without repair') -and $S3.Contains('Exact AMD DVR is already healthy; retained without MSI repair')) 'Healthy exact target MSI products are retained instead of needlessly reconfigured.'
Check 'RC2ZP_STAGE2_RESULT_CONTRACT' ($S2.Contains('stage2-result.json') -and $S2.Contains('LEGIONGO_STAGE_INVOCATION_ID') -and $S2.Contains('Write-Stage2Result') -and $Launcher.Contains('Stage 2 result contract agrees') -and $Launcher.Contains('Stage 2 process/result disagreement')) 'Stage 2 now has an invocation-bound child result contract; process exit, child status/detail, and checkpoint must agree.'
Check 'RC2ZP_STAGE2_REBOOT_COMMAND_MARKER' ($S2.Contains('Set-StateProperty $State ''DriverRebootCommandIssuedAt'' (Get-Date).ToString(''o'')') -and $S2.Contains('Set-StateProperty $State ''DriverRebootCommandIssuedAt'' ''''') -and $S2.Contains('Write-Stage2Result -Status ''RebootRequired'' -ExitCode 2')) 'Stage 2 records an explicit reboot-command marker and clears it before a later ReadyForInstall attempt.'
Check 'RC2ZP_STAGE2_REBOOT_EXIT_NORMALIZATION' ($Launcher.Contains('$Code -eq 0 -and $Stage2ContractCode -eq 2 -and $Stage2ContractStatus -eq ''RebootRequired''') -and $Launcher.Contains('$BoundaryCheckpoint -eq ''AwaitingNormalSigningReboot''') -and $Launcher.Contains('$BoundaryState.PSObject.Properties[''DriverRebootCommandIssuedAt'']') -and $Launcher.Contains('$Stage2EffectiveCode = 2') -and $Launcher.Contains('Stage 2 reboot handoff accepted')) 'Stage 2 shutdown exit normalization is limited to an invocation-bound RebootRequired contract at AwaitingNormalSigningReboot with an explicit reboot-command marker.'
Check 'RC2ZP_STAGE2_ZERO_EXIT_EXPECTED_CHECKPOINT' ($Launcher.Contains('''ReadyForInstall'' { if($StageFile -eq ''02-Install-Driver-And-Verify-Normal-Signing.ps1''){ return ''DriverComplete'' } }')) 'If Stage 2 ever legitimately returns zero from ReadyForInstall, the parent requires DriverComplete instead of reporting a blank expected checkpoint.'
Check 'RC2ZP_FAILURE_CODE_INTEGRITY_NORMALIZATION' ($Launcher.Contains('function Invoke-FailureCodeIntegrityRecovery') -and $Launcher.Contains('Get-TestSigningConfigured') -and $Launcher.Contains('Get-NoIntegrityChecksConfigured') -and $Launcher.Contains('& bcdedit.exe /set testsigning off') -and $Launcher.Contains('& bcdedit.exe /set nointegritychecks off') -and $Launcher.Contains('Failure recovery could not prove Test Signing and nointegritychecks are both configured off.')) 'Any hard launcher failure inspects BCD and, if needed, configures Test Signing and nointegritychecks OFF before recovery completes.'
Check 'RC2ZP_FAILURE_RECOVERY_REBOOT_NO_RETRY' ($Launcher.Contains('$CodeIntegrityRecovery.RecoverySucceeded -and $CodeIntegrityRecovery.RebootRequired') -and $Launcher.Contains('No installer resume task is armed; the failed workflow will NOT automatically retry.') -and $Launcher.Contains('Restart-Computer -Force')) 'A failure that normalizes BCD reboots only after resume-task removal and never automatically retries the failed installer.'
Check 'RC2ZP_FAILURE_EVIDENCE_INCLUDES_STAGE2' ($Launcher.Contains('@(''workflow-state.json'',''stage2-result.json'',''stage3-result.json'',''stage4-result.json'',''one-click-consent.json'')')) 'Failure evidence now preserves the Stage 2 invocation result contract when available.'
Check 'RC2ZP_STAGE3_RESULT_CONTRACT' ($S3.Contains('stage3-result.json') -and $S3.Contains('LEGIONGO_STAGE_INVOCATION_ID') -and $Launcher.Contains('Stage 3 result contract agrees') -and $Launcher.Contains('ExpectedInvocation=')) 'Stage 3 child exit, invocation-specific result contract, and launcher interpretation must agree.'
Check 'RC2ZP_STAGE3_REBOOT_COMMAND_MARKER' ($S3.Contains('Set-StateProperty $State ''SoftwareRebootCommandIssuedAt'' (Get-Date).ToString(''o'')') -and $S3.Contains('Set-StateProperty $State ''SoftwareRebootCommandIssuedAt'' ''''') -and $S3.Contains('Write-Stage3Result -Status ''RebootRequired'' -ExitCode 2')) 'Stage 3 records an explicit reboot-command marker immediately before the authorized automatic reboot and resets it before a later same-boot retry.'
Check 'RC2ZP_STAGE3_REBOOT_EXIT_NORMALIZATION' ($Launcher.Contains('$Code -eq 0 -and $ContractCode -eq 2 -and $ContractStatus -eq ''RebootRequired''') -and $Launcher.Contains('$BoundaryCheckpoint -eq ''AwaitingSoftwareReboot''') -and $Launcher.Contains('$BoundaryState.PSObject.Properties[''SoftwareRebootCommandIssuedAt'']') -and $Launcher.Contains('$EffectiveCode = 2') -and $Launcher.Contains('Stage 3 reboot handoff accepted')) 'The launcher narrowly normalizes the Windows shutdown race only for an invocation-bound Stage 3 RebootRequired contract at the saved software-reboot boundary with an explicit reboot-command marker.'
Check 'RC2ZP_STAGE3_REBOOT_MISMATCH_FAIL_CLOSED' ($Launcher.Contains('if (-not $NormalizedRebootShutdownExit)') -and $Launcher.Contains('throw "Stage 3 process/result disagreement. ProcessExit=$Code ContractExit=$ContractCode Status=$ContractStatus"')) 'All Stage 3 process/contract mismatches outside the exact reboot-shutdown exception still fail closed.'
Check 'RC2ZP_IDEMPOTENT_CATALOG_DISPOSITION' ($S2.Contains('RetainedPreexistingExactFinal') -and $S2.Contains('Stage1Candidate') -and $S2.Contains('ExpectedActiveCatalogSHA256') -and $S2.Contains('PreviousCatalogSHA256')) 'Stage 2 records whether Windows activated the new Stage 1 catalog or retained an already-installed exact-final catalog.'
Check 'RC2ZP_FINAL_AUDIT_EXPECTED_CATALOG' ($S4.Contains('ACTIVE_CATALOG_DISPOSITION') -and $S4.Contains('ACTIVE_CATALOG_EXPECTED_HASH') -and $S4.Contains('ACTIVE_CATALOG_SIGNER_TRUST_NO_PRIVATE_KEY') -and $S4.Contains('ACTIVE_CATALOG_INF_MEMBERSHIP') -and -not $S4.Contains('ACTIVE_CATALOG_MATCHES_STAGE1')) 'Final audit validates the catalog identity selected by Stage 2 instead of assuming an identical INF must replace an existing Driver Store catalog.'
Check 'RC2ZP_STAGE4_RESULT_CONTRACT' ($S4.Contains('stage4-result.json') -and $S4.Contains('Write-Stage4Result') -and $Launcher.Contains('Stage 4 result contract agrees') -and $Launcher.Contains('Stage 4 process/result disagreement')) 'Final audit pass/fail is bound to an invocation-specific result contract and cannot be converted into launcher success by an inconsistent process exit code.'
Check 'RC2ZP_EXACT_UNUSED_SIGNER_CLEANUP' ($Common.Contains('function Ensure-ExactPublicSignerTrustRemoved') -and $Common.Contains('Get-CertificateMatches -StorePath ''Cert:\LocalMachine\Root'' -Thumbprint $Thumbprint') -and $Common.Contains('Get-CertificateMatches -StorePath ''Cert:\LocalMachine\TrustedPublisher'' -Thumbprint $Thumbprint') -and $Common.Contains('Remove-Item -LiteralPath ([string]$Cert.PSPath)') -and $Common.Contains('Refusing to remove signer trust because the Stage 1 signer equals the expected active catalog signer.')) 'Unused current-run trust cleanup is restricted to the exact Stage 1 thumbprint and refuses to remove the retained active signer.'
Check 'RC2ZP_SIGNER_CLEANUP_RESUME_SAFE' ($S2.Contains('Apply-SignerTrustPolicy -State $State') -and $S2.Contains('Verified pre-reboot driver checkpoint saved before signer-trust policy') -and $S2.Contains("'UnusedStage1SignerRemoved'") -and $S2.Contains('Candidate catalog hash changed from the exact Stage 1 signed workspace.')) 'Stage 2 checkpoints active catalog identity before cleanup and can safely resume when the inactive candidate signer is already untrusted/absent.'
Check 'RC2ZP_UNUSED_SIGNER_FINAL_AUDIT' ($S4.Contains('STAGE1_SIGNER_TRUST_DISPOSITION') -and $S4.Contains('STAGE1_ACTIVE_SIGNER_RELATION') -and $S4.Contains('UNUSED_STAGE1_SIGNER_ABSENT') -and -not $S4.Contains('PUBLIC_SIGNER_TRUST_NO_PRIVATE_KEY')) 'Final audit distinguishes active signer trust from unused current-run signer absence.'
Check 'RC2ZP_FINAL_EVIDENCE_BUNDLE' ($Launcher.Contains('Prepare-FinalEvidenceContracts') -and $Launcher.Contains("'stage2-result.json'") -and $Launcher.Contains("'stage3-result.json'") -and $Launcher.Contains("'stage4-result.json'") -and $Launcher.Contains('Copy-RelevantLauncherEvidence') -and $Launcher.Contains("'Launcher-Logs'")) 'Launcher packages Stage 2/3/4 result contracts and the relevant launcher session logs into the Stage 4 evidence directory.'
Check 'RC2ZP_LAUNCHER_EVIDENCE_MARKERS' ($Launcher.Contains("'Stage 2 result contract agrees'") -and $Launcher.Contains("'Stage 3 result contract agrees'") -and $Launcher.Contains("'Stage 4 result contract agrees'") -and $Launcher.Contains("'One-command workflow marked complete.'") -and $Launcher.Contains("workflow returned to SoftwareComplete so Stage 4 can be safely rerun")) 'Evidence packaging requires Stage 2/3/4 parent-consumption markers and fails closed back to SoftwareComplete if final evidence cannot be completed.'
Check 'RC2ZP_COMBINED_TESTSIGN_REBOOT_GATE' ($S1.Contains("Enable temporary Windows Test Signing and restart Windows now?") -and $S1.Contains('Confirm-ManagedOrInteractive') -and $S1.Contains('Restart requested by the same consent gate that enabled Test Signing.') -and -not $S1.Contains("Enable temporary Windows Test Signing for the local catalog install?")) 'The Test Signing change and immediate reboot consume the already-recorded one-click authorization without adding another managed Y/N prompt.'

Check 'RC2ZP_LAUNCHER_RESULT_ERROR_FORMAT_SAFE' ($Launcher.Contains('catch { throw (''Stage 3 result contract is unreadable after process exit {0}: {1}'' -f $Code,$_.Exception.Message) }') -and -not $Launcher.Contains('process exit $Code:')) 'Launcher result-contract errors avoid the invalid expandable-string variable-colon form.'

Check 'RC2ZP_PS51_NO_FOREACH_PIPE' (-not ($S3 -match '(?m)^\s*\}\s*\|')) 'Stage 3 contains no Windows PowerShell 5.1 close-brace pipeline parser hazard.'
Check 'RC2ZP_STAGE3_NO_AUTOMATIC_ARGS_LOCAL' ($S3.Contains('$MsiArguments = @(') -and -not ($S3 -match '(?im)^\s*\$Args\s*=')) 'Stage 3 uses a normal local MSI argument variable and never assigns PowerShell automatic $Args.'
...
Check 'PUBLIC_CMD_USES_VALIDATED_GATE' ($PublicCmd.Contains('Run-Validated-LegionGo-AMD-26.7.1-Public-Beta-v3.1.ps1') -and -not $PublicCmd.Contains('-File "%~dp0Install-LegionGo-AMD-26.7.1.ps1"')) 'The public CMD entrypoint goes through the independent parser gate.'
Check 'RC2Q_PORTABLE_CACHE_REUSE' ($Common.Contains('C:\ProgramData\LegionGo-AMD-26.7.1-v3.0-RC2q\Tools\WindowsKits-Portable')) 'RC2zp can reuse the exact pinned portable Microsoft tool cache from the RC2q field run.'
Check 'RC2P_PORTABLE_CACHE_REUSE' ($Common.Contains('C:\ProgramData\LegionGo-AMD-26.7.1-v3.0-RC2p\Tools\WindowsKits-Portable')) 'RC2zp can reuse the exact pinned portable Microsoft tool cache from the successful RC2p field run.'
Check 'RC2O_PORTABLE_CACHE_REUSE' ($Common.Contains('C:\ProgramData\LegionGo-AMD-26.7.1-v3.0-RC2o\Tools\WindowsKits-Portable')) 'RC2zp can reuse an exact pinned RC2o portable Microsoft tool cache if present.'
Check 'RC2N_PORTABLE_CACHE_REUSE' ($Common.Contains('C:\ProgramData\LegionGo-AMD-26.7.1-v3.0-RC2n\Tools\WindowsKits-Portable')) 'RC2zp can reuse an exact pinned RC2n portable Microsoft tool cache if present.'
Check 'RC2M_PORTABLE_CACHE_REUSE' ($Common.Contains('C:\ProgramData\LegionGo-AMD-26.7.1-v3.0-RC2m\Tools\WindowsKits-Portable')) 'RC2zp can reuse the exact pinned portable Microsoft tool cache from the fully successful RC2m regression.'
Check 'RC2L_PORTABLE_CACHE_REUSE' ($Common.Contains('C:\ProgramData\LegionGo-AMD-26.7.1-v3.0-RC2l\Tools\WindowsKits-Portable')) 'RC2zp can reuse the exact pinned portable Microsoft tool cache downloaded and smoke-validated in the RC2l field run.'
Check 'RC2K_PORTABLE_CACHE_REUSE' ($Common.Contains('C:\ProgramData\LegionGo-AMD-26.7.1-v3.0-RC2k\Tools\WindowsKits-Portable')) 'RC2zp can reuse the exact pinned portable Microsoft tool cache field-validated in RC2k.'
...
Check 'RC2ZP_PUBLIC_EXACTLY_TWO_YN' ($LauncherYnCount-eq2 -and $Launcher.Contains('This is a one-click installer that will install AMD graphics driver 26.7.1 on the Lenovo Legion Go. Proceed?') -and $Launcher.Contains('This installer will automatically restart your device as required and continue installation after each reboot.') -and -not $S1.Contains('Confirm-YesNo') -and -not $S2.Contains('Confirm-YesNo') -and -not $S3.Contains('Confirm-YesNo')) ("Public launcher Y/N call count={0}; managed stages contain no direct Confirm-YesNo calls." -f $LauncherYnCount)
Check 'RC2ZP_TWO_YN_AT_FRESH_RUN_FRONT' ($Launcher.IndexOf('    Assert-OneClickSecureBootReady') -lt $Launcher.IndexOf('    $ToolkitRoot = Initialize-PersistentToolkit') -and $Launcher.IndexOf('        if (-not (Request-OneClickConsent))') -lt $Launcher.IndexOf('    $ToolkitRoot = Initialize-PersistentToolkit') -and $Launcher.IndexOf('    $ToolkitRoot = Initialize-PersistentToolkit') -lt $Launcher.IndexOf("    if (`$InitialCheckpoint -eq 'NotStarted') { `$FirstRunInstaller = Resolve-ValidatedInstallerForFirstRun")) 'On a fresh run, Secure Boot is checked first, then the only two Y/N confirmations occur before the long full preflight/copy/installer-hash workflow begins.'
Check 'RC2ZP_AUTOMATIC_REBOOTS_AFTER_INITIAL_CONSENT' ($Common.Contains('function Confirm-ManagedOrInteractive') -and $Common.Contains('Test-OneClickConsent') -and $S1.Contains('Confirm-ManagedOrInteractive') -and $S2.Contains('Confirm-ManagedOrInteractive') -and $S3.Contains('Confirm-ManagedOrInteractive') -and $Common.Contains('AutomaticRebootsApproved=$true')) 'After the two initial confirmations, dependency/change gates and all workflow-required reboots are authorized automatically in managed mode.' 
...
if(-not [string]::IsNullOrWhiteSpace($ResultPath)){
    $ResultObject=[ordered]@{
        Schema='LegionGo-PublicBeta-v3.1-PreflightResult-v1'
        Passed=($Failed.Count-eq0)
        FailedChecks=[int]$Failed.Count
        FailedCheckNames=@($Failed | ForEach-Object { [string]$_.Name })
        FailedCheckDetails=@($Failed | ForEach-Object { [pscustomobject]@{Name=[string]$_.Name;Detail=[string]$_.Detail} })
