...
.DESCRIPTION
    ...
    First run:
      ...
      - registers AMD's exact Microsoft-signed catalog;
...
$EmbeddedPayload = [ordered]@{
    ...
    'Phase-01-Install-Corrected-Driver.ps1' = [ordered]@{
        SHA256 = '34CE8C46493383117A6EF90AAA87294298FED3D86217A37FD28513C24F2C8506'
        Utf8Bom = $true
        LineEnding = 'CRLF'
...
Actions:
  ...
  - Register AMD's exact Microsoft-signed original catalog through WinTrust.
...
function Register-OfficialCatalog {
    ...
    if (-not ('LegionGoCatAdminAddNative' -as [type])) {
        Add-Type -TypeDefinition @'
using System;
using System.Runtime.InteropServices;

public static class LegionGoCatAdminAddNative
{
    [DllImport(
        "wintrust.dll",
...
@'
    ...
    try {
        $Acquired =
            [LegionGoCatAdminAddNative]::CryptCATAdminAcquireContext2(
                [ref]$AdminContext,
                [IntPtr]::Zero,
                'SHA256',
                [IntPtr]::Zero,
        ...
        if (-not $Acquired) {
            ...
            throw "CryptCATAdminAcquireContext2 failed: $Message ($Code)"
        }

        $CatalogContext =
            [LegionGoCatAdminAddNative]::CryptCATAdminAddCatalog(
                $AdminContext,
                $CatalogPath,
                $null,
                0
        ...
        if ($CatalogContext -eq [IntPtr]::Zero) {
            $Code = [Runtime.InteropServices.Marshal]::GetLastWin32Error()
            $Message = [ComponentModel.Win32Exception]::new($Code).Message
            throw "CryptCATAdminAddCatalog failed: $Message ($Code)"
        }
    }
    finally {
        if ($CatalogContext -ne [IntPtr]::Zero) {
            [void][LegionGoCatAdminAddNative]::CryptCATAdminReleaseCatalogContext(
                $AdminContext,
                $CatalogContext,
                0
            )
        ...
        if ($AdminContext -ne [IntPtr]::Zero) {
            [void][LegionGoCatAdminAddNative]::CryptCATAdminReleaseContext(
                $AdminContext,
                0
            )
        }
...
@'
    ...
    if ($OfficialCatalogAlreadyRegistered) {
        ...
        Register-OfficialCatalog -CatalogPath $ResolvedOfficialCatalog
    }
